...
The analyzer lower-cases text but keeps diacritics, so *orçamento* and
*orcamento* are different words. Write queries with the proper accents:
...
If your users type without accents, strip them from both the text and the
queries before they reach the index, for example with
`iconv(x, to = "ASCII//TRANSLIT")` or `stringi::stri_trans_general(x,
"Latin-ASCII")`, and keep the original text in a separate stored column for
display.
